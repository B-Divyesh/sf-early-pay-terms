...
test('@claim:demo-isolation opens a populated sample and cannot read production storage', async ({ browser }) => {
  ...
  const demo = await browser.newContext({ viewport: { width: 390, height: 844 } }); const dp = await demo.newPage();
...
test('@claim:free-core calculates, creates a payment card, and exports while Plus is locked', async ({ page }) => {
  await page.goto('/demo'); await page.locator('#terms-form[data-ready="true"]').waitFor();
...
test('@claim:draft-persistence keeps edits in the demo database only', async ({ browser }) => {
  const demo = await browser.newContext(); const dp = await demo.newPage(); await dp.goto('/demo'); await dp.locator('#terms-form[data-ready="true"]').waitFor();
  await dp.getByLabel('Invoice reference').fill('PERSIST-DEMO'); await dp.waitForTimeout(350); await dp.reload(); await expect(dp.getByLabel('Invoice reference')).toHaveValue('PERSIST-DEMO');
  const real = await browser.newContext(); const rp = await real.newPage(); await rp.goto('/'); await rp.locator('#terms-form[data-ready="true"]').waitFor(); await expect(rp.getByLabel('Invoice reference')).not.toHaveValue('PERSIST-DEMO');
  await demo.close(); await real.close();
});

test('@claim:plus-entitlements gates all three Plus tools and enables each with a valid cached license', async ({ browser }) => {
  const locked = await browser.newContext(); const lp = await locked.newPage(); await lp.goto('/demo'); await lp.locator('#terms-form[data-ready="true"]').waitFor();
  ...
  const licensed = await browser.newContext(); await licensed.addInitScript(() => { localStorage.setItem('sb_license:early-pay-terms', 'fixture'); localStorage.setItem('sb_license_verdict:early-pay-terms', JSON.stringify({ valid: true, checkedAt: Date.now() })); });
  const up = await licensed.newPage(); up.on('dialog', dialog => dialog.accept('Standard 2%'));
  await up.goto('/demo'); await up.locator('#terms-form[data-ready="true"]').waitFor(); await expect(up.locator('#license-state')).toContainText('Plus is active');
...
test('@claim:plus-sale-state renders no checkout or purchase action', async ({ page }) => {
  await page.goto('/'); await page.locator('#terms-form[data-ready="true"]').waitFor();
...
test('@claim:json-import restores an exported current calculation and saved fields', async ({ browser }) => {
  const source = await browser.newContext(); const sp = await source.newPage(); await sp.goto('/demo'); await sp.locator('#terms-form[data-ready="true"]').waitFor();
  await sp.getByLabel('Invoice reference').fill('IMPORTED-2048'); await sp.getByText('Names and note for the payment card').click(); await sp.getByLabel('Supplier name').fill('Imported Paper Ltd'); await sp.waitForTimeout(350);
  ...
  const target = await browser.newContext(); const tp = await target.newPage(); await tp.goto('/demo'); await tp.locator('#terms-form[data-ready="true"]').waitFor();
...
test('@claim:product-boundary presents calculation tools without invoice, collection, or accounting actions', async ({ page }) => {
  await page.goto('/'); await page.locator('#terms-form[data-ready="true"]').waitFor(); await expect(page.getByRole('heading', { name: 'What this calculator does not do' })).toBeVisible(); await expect(page.getByText('It does not decide what is legal or create accounting entries.')).toBeVisible();
...
test('@claim:discount-bases applies each advertised discount rule', async ({ page }) => {
  await page.goto('/demo'); await page.locator('#terms-form[data-ready="true"]').waitFor(); await page.locator('#netAmount').fill('100.25'); await page.locator('#taxAmount').fill('19.25');
...
test('Privacy then browser Back restores the calculator fragment, focus, scroll, and announcement', async ({ page }) => {
  await page.goto('/'); await page.locator('#terms-form[data-ready="true"]').waitFor(); await page.getByRole('link', { name: 'Calculator', exact: true }).click();
...
test('every real route has complete metadata and the shared main navigation', async ({ page }) => {
  ...
  for (const [route, title] of [['/', 'Early Pay Terms — invoice discount calculator'], ['/demo', 'Demo — Early Pay Terms'], ['/privacy/', 'Privacy — Early Pay Terms'], ['/terms/', 'Terms — Early Pay Terms']] as const) {
    await page.goto(route); await expect(page).toHaveTitle(title); await expect(page.locator('html')).toHaveAttribute('lang', 'en'); await expect(page.locator('main')).toHaveCount(1); await expect(page.locator('h1')).toHaveCount(1);
