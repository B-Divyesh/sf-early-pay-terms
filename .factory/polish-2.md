...
| Finding | Change made | Evidence |
| --- | --- | --- |
| Browser lifecycle could fail after an unrelated spec | Added `tests/e2e/fixtures.ts`. Every browser spec now launches, owns, and closes a fresh Chromium process, isolated browser context, and page; extra contexts use the active project’s exact viewport settings. No browser, context, page, cookies, IndexedDB, route handler, or page event listener is shared with another spec. | Fresh clone `/tmp/ept-polish2-retry1-aH4owc`: every claim command passed, then `CI=1 npm run test:stable` passed two consecutive full runs (7/7 Vitest and 42/42 Playwright each). |
